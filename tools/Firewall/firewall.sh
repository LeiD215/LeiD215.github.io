...
# ==============================================================================
# 🛡️ Linux 自动化防火墙安全管理脚本 (高级综合版)
# 支持系统: CentOS, Debian, Ubuntu, Rocky Linux, AlmaLinux 等
# 功能特性: 自动识别OS、解冲突卸载、SSH防锁死、端口开关管理、
...
manage_installation() {
    ...
    echo "✅ 新防火墙系统 (${CURRENT_FW^^}) 安装并启动成功。"
}
...
enable_ip_forwarding() {
    # 根据用户选择的 IP 栈，智能地使能内核配置，并追加 accept_ra=2 防 IPv6 断网
    ...
    if [ "$IP_VER" = "6" ] || [ "$IP_VER" = "both" ]; then
        if [ "$(sysctl -n net.ipv6.conf.all.forwarding 2>/dev/null)" -ne 1 ]; then
            ...
            # 健壮性加固：防止开启 IPv6 转发后导致部分动态机房的网络无法自动获取网关(RA)而断网
    ...
    if [ "$CURRENT_FW" = "ufw" ]; then
        for p in "${protos[@]}"; do
            if [ "$IP_VER" = "4" ]; then
                ...
                # both 情况下不指定 ip 版本，UFW 默认双栈同时生成规则
...
manage_forwarding() {
    ...
    choose_proto
    choose_ip_version
    enable_ip_forwarding  # 触发上文改进的智能双栈内核转发使能
...
# 智能兜底：如果用户未触发安装动作，自动绑定当前系统中正在存活运转的防火墙
